...
<header>
        <div class="logo">
            ...
            <a class="navItem" >Comprar ticket</a>
            </nav>
     </div>
</header>
<main>
...
total=String.valueOf(Math.ceil(cant*rate*entrada));
...
try {
	...
	ResultSet rsId = stm.executeQuery("SELECT MAX(id_ticket) FROM `tickets`");
	while(rsId.next()) {
		
	int idNum = rsId.getInt(1);
	
	query = "UPDATE `tickets` SET `nombre`='"+nombre+"',`apellido`='"+apellido+"',`email`='"+email+"',`cantidad`='"+cantidad+"',`cat`='"+cat+"' WHERE id_ticket = "+idNum+"";	
